...
W = ((objs, current, next, lastFrame, textures, perspective, draw, render, lerp, transition, dist) => {
  ...
  draw = (now, dt, v, i, transparent = []) => {
    ...
    // Clear canvas
    W.gl.clear(16640 /* W.W.gl.COLOR_BUFFER_BIT | W.W.gl.DEPTH_BUFFER_BIT */);
    ...
    W.gl.uniform3f(
      ...
      lerp('light','x'), lerp('light','y'), lerp('light','z')
    );
  ...
  render = (object, dt, buffer) => {
    ...
    if (object.b.id) {
      ...
      W.gl.bindTexture(3553 /* TEXTURE_2D */, textures[object.b.id]);
    ...
    //console.log(next[object.n].M || W.transition(object.n));
    next[object.n].m = transition(object.n);

    // If the object is in a group:
    if(next[object.g]){

      // premultiply the model matrix by the group's model matrix.
      next[object.n].m.preMultiplySelf(next[object.g].m);
    }
    ...
    W.gl.uniformMatrix4fv(
      ...
      false,
      (next[object.n].M || next[object.n].m).toFloat32Array()
    );
  ...
  lerp = (item, property) => 
    next[item]?.t
    ? current[item][property] + (next[item][property] - current[item][property]) * (next[item].f / next[item].t)
    : next[item][property];
  ...
  transition = (item, m = new DOMMatrix) =>
    next[item]
    ? m
      .translateSelf(lerp(item, 'x'), lerp(item, 'y'), lerp(item, 'z'))
      .rotateSelf(lerp(item, 'rx'),lerp(item, 'ry'),lerp(item, 'rz'))
      .scaleSelf(lerp(item, 'w'),lerp(item, 'h'),lerp(item, 'd'))
    : m;
    
  // Compute the distance squared between two objects (useful for sorting transparent items)
  dist = (a, b = next.camera) => a && b ? (b.m.m41 - a.m.m41)**2 + (b.m.m42 - a.m.m42)**2 + (b.m.m43 - a.m.m43)**2 : 0;
  ...
  return {
    ...
    reset: (canvas, t) => {
      ...
      next = {};        // Objects next states
      textures = {};    // Textures list
      perspective =     // Perspective matrix (fov: .5rad, aspect: width/height, near: 1, far: 1000)
        new DOMMatrix([
  ...
  setState: (state, type, texture, i, vertices, A, B, C, AB, BC, normal) => {
    
    //console.log(state, type, current, next);

    // Custom name or default name ('o' + auto-increment)
    state.n ||= 'o' + objs++;
    
    // If a new texture is provided, build it and save it in W.textures
    if(state.b && state.b.id && state.b.width && !textures[state.b.id]){
      ...
      textures[state.b.id] = texture;
    }
    
    // Save object's type,
    // merge previous state (or default state) with the new state passed in parameter,
    // and reset f (the transition timer)
    state = {type, ...(current[state.n] = next[state.n] || {w:1, h:1, d:1, x:0, y:0, z:0, rx:0, ry:0, rz:0, b:'888'}), ...state, f:0};
    ...
    delete: (t, delay) => setTimeout(()=>{ delete next[t.n] }, delay || 1),
